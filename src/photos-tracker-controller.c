...
photos_tracker_controller_cursor_next (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
  ...
  _photos_debug_print (PHOTOS_DEBUG_TRACKER, "Query Cursor: %" G_GINT64_FORMAT, (now - priv->last_query_time) / 1000000);
...
photos_tracker_controller_set_query_status (PhotosTrackerController *self, gboolean query_status)
{
  ...
  if (query_status)
    ...
    {
      _photos_debug_print (PHOTOS_DEBUG_TRACKER, "Query Elapsed: %" G_GINT64_FORMAT, (now - priv->last_query_time) / 1000000);
